...
namespace NodeHDF5 {
    ...
    private:
        ...
        explicit PacketTable(hid_t packetTableID, unsigned int nmembers) : packetTableID(packetTableID), nmembers(nmembers), p_data(new char*[nmembers]) {
        ...
        static void next(const v8::FunctionCallbackInfo<v8::Value>& args) {
            ...
            for (unsigned int index = 0; index < obj->nmembers; index++) {
                record->Set(args.This()->Get(String::NewFromUtf8(v8::Isolate::GetCurrent(), "record"))->ToObject()->GetOwnPropertyNames()->Get(index), String::NewFromUtf8(v8::Isolate::GetCurrent(), obj->p_data[index]));
            }
...
//                std::cout << "crating " << (*table_name) << " " << (*record_name) << std::endl;
                H5Tinsert(compoundID, *record_name, index * sizeof (char*), vlenID);
                String::Utf8Value record_value(record->Get(record->GetOwnPropertyNames()->Get(index))->ToString());
//                std::cout << index << " row1 " << *record_value << std::endl;
                obj->p_data[index] = new char[std::strlen(*record_value) + 1];
                std::strcpy(obj->p_data[index], *record_value);
            }
            H5Tpack(compoundID);
            ...
            H5Tclose(compoundID);
            H5Tclose(vlenID);
            herr_t err = H5PTcreate_index(obj->packetTableID);
            if (err < 0) {

            }
//            std::cout << obj->packetTableID << " obj->packetTableID " << obj->nmembers << " " << obj->p_data[0] << std::endl;
            err = H5PTappend(obj->packetTableID, (hsize_t) 1, obj->p_data.get());
            if (err < 0) {
...
//            std::cout << obj->packetTableID << " appended " << obj->nmembers << " " << err << std::endl;
        ...
        static void read_table(const v8::FunctionCallbackInfo<Value>& args) {
            ...
            err = H5PTcreate_index(packetTableID);
            if (err < 0) {
...
//                    std::cout << H5Tget_nmembers(type) << " pt type=" << H5Tis_variable_str(type) << std::endl;
                    for (int memberIndex = 0; memberIndex < nmembers; memberIndex++) {
                        ...
                        if (H5Tis_variable_str(memberType)) {
                            record->Set(String::NewFromUtf8(v8::Isolate::GetCurrent(), H5Tget_member_name(type, memberIndex)), String::NewFromUtf8(v8::Isolate::GetCurrent(), "huh"));
                        } else {
                            record->Set(String::NewFromUtf8(v8::Isolate::GetCurrent(), H5Tget_member_name(type, memberIndex)), String::NewFromUtf8(v8::Isolate::GetCurrent(), "huh2"));
                        }
                        H5Tclose(memberType);
                    }
                    H5Tclose(type);
